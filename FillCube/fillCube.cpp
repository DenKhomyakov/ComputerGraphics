...
double* MatrixMultiplication(double matrixA[][4], int rowsA, double matrixB[][4], int rowsB) {
    ...
    for (int i = 0; i < rowsA; i++) {
        for (int k = 0; k < 4; k++) {
            ...
            for (int m = 0; m < 4; m++) {
                resultMatrix[i * 4 + k] += matrixA[i][m] * matrixB[m][k];
            }
        }
    }
...
void DrawCube(double transformedVertices[][4], int* cubeFace, int cubeColor) {
    int cubeColors[] = { RED, BLUE, YELLOW, GREEN, BROWN, MAGENTA };
    setcolor(BLACK);
    setfillstyle(SOLID_FILL, cubeColors[cubeColor]);

    int newCubeVertices[10];

    for (int i = 0; i < 5; i++) {
        newCubeVertices[i * 2] = screenCenterX + (int)(transformedVertices[cubeFace[i]][0] * scaleCoefficient);
        newCubeVertices[i * 2 + 1] = screenCenterY - (int)(transformedVertices[cubeFace[i]][1] * scaleCoefficient);
    }
